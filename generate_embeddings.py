...
def perform_similarity_search(input_file, output_index_bin, query=None, k=5):
    # Read the input Parquet file and create embeddings
    df = pd.read_parquet(input_file)
    embeddings = get_sentence_embeddings(df['chunk_content'].tolist())

    # Save the embeddings to 'embeddings.npy' for later use
    np.save('embeddings.npy', embeddings)
    print("Embeddings saved to embeddings.npy successfully!")

    # Create the HNSW index and save it
    index = create_hnsw_index(embeddings)
    index.save_index(output_index_bin)
    print(f"HNSW index created and saved successfully to {output_index_bin}!")
